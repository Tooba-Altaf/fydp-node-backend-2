...
exports.registerManufacturerValidation=[
    ...
    check('name').trim().not().isEmpty().withMessage('Password is empty').isLength({min:3}).withMessage("Name should be minimum 3 characters long"),
    ...
    check('contact').trim().not().isEmpty().withMessage(" contact is empty").isLength({min:13}).matches(/^[0-9]+$/).withMessage("Contact should be 13 numbers long"),
]